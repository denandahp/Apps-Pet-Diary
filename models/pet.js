...
class petModel {
  ...
  async listMedcheck (pet_id, note_id) {
    try{
      let res;
      if (pet_id === 'all') {
        res = await pool.query('SELECT id, pet_id, pet_checkup_date, pet_note, pet_photo, updated_at from ' + dbPetcheckup + ' ORDER BY pet_id ASC')
      } else if(pet_id != undefined && note_id == 'all'){
        //All note by id pet
        res = await pool.query('SELECT id, pet_id, pet_checkup_date, pet_note, pet_photo, updated_at from ' + dbPetcheckup + ' where pet_id = $1 AND pet_checkup_date IS NOT NULL ORDER BY pet_id ASC', [pet_id]);
      } else{
        //Detail note by id pet and note id
        res = await pool.query('SELECT id, pet_id, pet_checkup_date, pet_note, pet_photo, updated_at from ' + dbPetcheckup + ' where pet_id = $1 AND id = $2 AND pet_checkup_date IS NOT NULL ORDER BY pet_id ASC', [pet_id, note_id]);
      }
  ...
  async listnote (pet_id, note_id) {
    try{
      let res;
      if (pet_id === 'all') {
        res = await pool.query('SELECT id, pet_id, pet_height, pet_weight, pet_temperature, pet_note, pet_photo, updated_at from ' + dbPetcheckup + ' ORDER BY pet_id ASC')
      } else if(pet_id != undefined && note_id == 'all'){
        console.log("Get All Note Pet By Id_User");
        //All note by id pet
        res = await pool.query('SELECT id, pet_id, pet_height, pet_weight, pet_temperature, pet_note, pet_photo, updated_at from ' + dbPetcheckup + ' where pet_id = $1 AND pet_checkup_date IS NULL ORDER BY pet_id ASC', [pet_id]);
      } else{
        console.log("Get Note Pet By Id_User");
        //Detail note by id pet and note id
        res = await pool.query('SELECT id, pet_id, pet_height, pet_weight, pet_temperature, pet_note, pet_photo, updated_at from ' + dbPetcheckup + ' where pet_id = $1 AND id = $2 AND pet_checkup_date IS NULL ORDER BY pet_id ASC', [pet_id, note_id]);
      }
